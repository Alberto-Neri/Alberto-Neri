...
m0 = 0.5; %5.2934e-02
h0 = 0.596;
n0 = 0.3176; %3.1768e-01
y0 = [V0, m0, h0, n0];
%y0 = [V0, 5.2934e-02, 5.9611e-01, 3.1768e-01]; %condizioni resting

%SE VOGLIO VEDERE CHE VA A RIPOSO METTO V0=-10 e m0=0.1
%poi apro pdf e vedo come p la varietà

[tout,yout] = ode15s(@fHH_es7_vv,t,y0,[],k); %simulo tutto anche m,h !!
Vout = yout(:,1);
mout = yout(:,2);
...
% V = yout(:,1);
[V,m] = meshgrid(-20:0.1:120, 0:0.01:1); %definisco i punti su cui voglio vedere le nullc  
...
nullc_V = gNa*m.^3*h0.*(V-VNa) + gK*n0^4.*(V-VK) + gL*(V-VL); %scrivo solo termine noto!!
nullc_m = m_inf;

figure(1)
subplot(2,1,1)
contour(V,m,nullc_V, [0 0])
hold on
plot(V(1,:),nullc_m(1,:))
xlabel('V')
ylabel('m')
