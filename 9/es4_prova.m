...
T = 30;  %serve tempo >1 se no non si vede niente
k=0.020; %sarebbe il tau --> variazione nel tempo
tempo=0:k:T;
nt=T/k;
...
%               Iapp(i,j) = 50;
%           end
          if (i >= length(x)-5 && tempo(j)<=1) %impulso da sx
              Iapp(i,j) = -100;
          end
  ...
  for kk=1:nt
    ...
    bb= Cm*v_j/k -(gNa_t.*m_t.^3.*h_t.*(v_j-VNa) + gK*n_t.^4.*(v_j-VK) + gL*(v_j-VL)) + Iapp(:,tindex);
